...
You’ve already set up Git, GitHub, Netlify, and Cursor IDE, and you have this repo checked out (e.g. via the sales-showcase how-to). This guide adds what you need for this repo: Node.js, open the repo in Cursor, and install project dependencies when prototype/ exists.
...
Run the .pkg and follow the steps.
Restart the terminal (or Cursor), then run:
...
Run it; leave “Add to PATH” checked.
Restart the terminal (or Cursor), then run:
node -v   # e.g. v20.x.x or v22.x.x
npm -v    # e.g. 10.x.x
...
Quick Checklist
 ...
 (When applicable) cd prototype && npm install run successfully
If Something Fails
